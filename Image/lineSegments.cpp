...
// OrientedLineSegment //
/////////////////////////
const double OrientedLineSegment::A=0.5;

const double OrientedLineSegment::B=1.0;

const double OrientedLineSegment::P=0.2;
...
namespace Image
{
	...
	std::istream &operator >> ( std::istream &stream , OrientedLineSegmentPairs &ols )
	{
		...
		ols.resize( sz );
		for( int i=0 ; i<sz ; i++ ) if( !( stream >> ols[i].first >> ols[i].second ) ) THROW( "Failed to read oriented line segment pair: %d" , (int)sz );
		return stream;
	}
}
